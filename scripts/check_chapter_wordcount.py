...
def check_all_chapters(directory: str, pattern: str = '第*.md', min_words: int = 3000) -> list:
    ...
    # 直接在目录下查找
    chapter_files.extend(sorted(dir_path.glob(pattern)))

    # 在卷子目录下查找
    for sub_dir in sorted(dir_path.iterdir()):
        if sub_dir.is_dir() and ('卷' in sub_dir.name):
            chapter_files.extend(sorted(sub_dir.glob(pattern)))
    ...
    for result in results:
        if result['exists']:
            ...
            if '卷' in parent:
                ...
                vol_name = '(根目录)'
            if vol_name not in volume_stats:
                volume_stats[vol_name] = {'count': 0, 'words': 0}
            volume_stats[vol_name]['count'] += 1
            volume_stats[vol_name]['words'] += result['word_count']

    print('\n' + '-' * 60)
    print(f'总计: {len(results)} 章 | {passed} 章达标 | {failed} 章不足 | 总字数: {total_words:,}')

    if len(volume_stats) > 1:
        print('\n按卷统计:')
        for vol_name, stats in volume_stats.items():
            print(f'  {vol_name}: {stats["count"]} 章, {stats["words"]:,} 字')
